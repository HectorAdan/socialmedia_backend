...
module.exports = function(app, connection, dir_name){
     ...
     app.get('/api/post-get-all', (req, res)=>{
        const sql  = "SELECT Posts.*, Users.firstName, Users.lastName FROM Posts, Users WHERE Posts.idUser = Users.idUser AND Posts.idStatus > 1 ORDER BY Posts.idPost DESC ";
    ...
    app.get('/api/post-get-all-by-user/:id', (req, res)=>{
        const {id} = req.params;
        const sql  = `SELECT Posts.*, Users.firstName, Users.lastName FROM Posts, Users WHERE Posts.idUser = Users.idUser AND Users.idUser = ${id}  ORDER BY Posts.idPost DESC `;
        connection.query(sql, (error, results) =>{
            if(error) throw error;
            if(results.length >0){
                const response = {
